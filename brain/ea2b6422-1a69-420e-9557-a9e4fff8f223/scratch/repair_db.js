...
async function repair() {
  console.log("--- REPARANDO CHAT DE OSCAR ---");
  try {
    const chatRef = doc(db, 'chats', 'order_FcGYX0V8tEfOvBzMZohn');
    await updateDoc(chatRef, {
      merchantId: 'eCVEN1NbWNgnLzMnU3EW',
      merchantName: 'Los socios'
    });
    console.log("¡Chat reparado con éxito! Vinculado a 'Los socios'.");
  } catch (error) {
    console.error("Error reparando chat:", error);
  }
}
